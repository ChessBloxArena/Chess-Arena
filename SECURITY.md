...
Use `ChessBlox Team <team@chessblox.invalid>`. Human account-specific GitHub noreply addresses are rejected because they link commits to an account. The checked-in `.gitidentity` records the team identity. Run `npm run setup:anonymous-git` after cloning. The guards validate effective author/committer values (including environment overrides), commit messages, outgoing annotated tags, and signatures. Commit/tag signing is disabled locally to avoid exposing signing identities. GitHub-generated merge and bot commits may carry a platform signature: the guard permits it only when GitHub verifies the signature and attributes the committer to its `web-flow` account with `noreply@github.com`. Unknown signatures and unavailable verification fail closed. This isolates Git author/committer attribution only. Pull-request authors, comments, organization membership, and other GitHub activity may still identify the publishing account. Review those surfaces before changing repository visibility; local hooks cannot anonymize platform records.
